...
class BinomialTree:
    ...
    def price(self, u: float, d: float, q: float) -> float:
        ...
        for step in range(self.steps - 1, -1, -1):
            # Calculate expected option value at this node
            option_values = (q * option_values[:-1] + (1 - q) * option_values[1:]) * self.discount
            
            # Asset prices at this node
            asset_prices = self.S * (u ** (self.steps - step - 1)) * (d ** (step + 1))
